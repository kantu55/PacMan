...
void Actor::Update(float deltaTime)
{
	...
	const glm::mat4 matR_ZY = glm::rotate(glm::mat4(1), rotation.z, glm::vec3(0, 0, -1));
	const glm::mat4 matR_XZY = glm::rotate(glm::mat4(1), rotation.x, glm::vec3(1, 0, 0));
...
StaticMeshActor::StaticMeshActor(const Mesh::FilePtr& m, const std::string& name, int health,
	const glm::vec3& position, const glm::vec3& rotation,
	const glm::vec3& scale) : Actor(name, health, position, rotation, scale), mesh(m)
{
...
void StaticMeshActor::Draw()
{
	...
	// アクターに設定された位置、回転、拡大率からモデル行列を計算
		...
		{
			// 使用するライトのインデックスをユニフォーム変数にコピー
			const Shader::ProgramPtr p = mesh->material[0].program;
			if (p)
			{
				p->Use();
