...
namespace Terrain
{
	...
	*/
	bool HeightMap::LoadFromFile(const char* path, float scale, float baseLevel)
	{
		...
		//画像データは下から上に向かって格納されるので、上下反転しながら高さデータに変換
		...
		for (int i = 0; i < 2; ++i)
		{
			// GL_RGBA8I   RGBAの4要素からなる8ビット符号付き整数
	...
	*/
	float HeightMap::Height(const glm::vec3& pos) const
	{
		...
		//「高さ」はfposが含まれる正方形の左上頂点を基準として計算するので、左上座標
		// マップは右下がプラス方向で、正方形の大きさは1.0*1.0とする
		// そうすることで、左上の頂点座標を求める計算は、単にfposの小数部を切り捨てるだけになる
		// ただし、右端と下端の1列は、その先に正方形がないことから左上座標として使えないため
		// 左上頂点座標はsize-2に制限する. 座標fposは
		//		(index.x, index.y), (index.x + 1, index.y)
		//		(index.x + 1, index.y), (index.x + 1, index.y + 1)
		...
		//  　 +--+
		//     +Y
		// 奥が-Y方向、手前が+Y方向で、1*1の正方形であることから
	...
	*/
		...
		Mesh::Buffer& meshBuffer, const char* meshName, const char* texName) const
	{
		...
		m.texture[1] = Texture::Image2D::Create("Res/Terrain_Plant.tga");
		m.texture[2] = Texture::Image2D::Create("Res/Terrain_Rock.tga");
		m.texture[3] = Texture::Image2D::Create("Res/Terrain_Soil.tga");
	...
	@retval false メッシュの作成に失敗
	*/
	bool HeightMap::CreaetWaterMesh(Mesh::Buffer& meshBuffer,
	...
	*/
	...
	@param cneterX 計算対象のX座標
	@param centerZ 計算対象のY座標
